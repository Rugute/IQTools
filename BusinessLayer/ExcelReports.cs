...
namespace BusinessLayer
{
  public class ExcelReports
  {
    ...
    public bool bPatSummaryBulkReport = false; 
      //KKirui - Bulk patient summary reports
    public DataTable DHISADX;
    public  DateTime fromDateMOH ;
    public DateTime ToDateMOH;
    ...
    public void MoH731 ( Hashtable pmARQueries, string locType, string dbSet, string cccSet, DataTable locTable, Boolean bySheet, ProgressBar pb )
    {
      ...
      theObject = new Entity ( );
      myParameters = pmARQueries;
      fromDateMOH = Convert.ToDateTime(myParameters["@FromDate"]);
      ToDateMOH = Convert.ToDateTime(myParameters["@ToDate"]);
         ...
         //     oWs.Unprotect(clsGbl.ReportPassword);
          ...
          if (xlSheet)
          {
              ...
              theDt = (DataTable)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams
                                      ...
                                      "WHERE a.QryID Is Not Null AND a.DeleteFlag IS NULL And b.CatID=" + mReport
              ...
              DataRow dr = (DataRow)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams
                                          ...
                                          "And a.DeleteFlag IS NULL AND b.CatID=" + mReport
                                          , ClsUtility.ObjectEnum.DataRow, clsGbl.PMMSType);
          ...
          else
          {
              ...
              DataRow dr = (DataRow)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams
                                          , "SELECT Count(QryID) Total FROM " +
                                          "aa_SBCategory a INNER JOIN  aa_Category b ON a.CatID = b.CatID WHERE a.QryID Is Not Null " +
                                          "And a.DeleteFlag IS NULL AND b.CatID=" + mReport, ClsUtility.ObjectEnum.DataRow, clsGbl.PMMSType);
          ...
          if (locType.ToLower() == "allrc" || locType.ToLower() == "allcr")
          {
              ...
              try
              {
                  ...
                  qryDt = (DataTable)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams
                      ...
                      "and aa_SBCategory.deleteflag is null", ClsUtility.ObjectEnum.DataTable, clsGbl.PMMSType);
                  qryDtr = qryDt.CreateDataReader();

                  DataRow dr = (DataRow)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams
                      , "SELECT Count(aa_Queries.QryID) Total FROM aa_Queries " +
                      "inner join aa_SBCategory on aa_Queries.qryid = aa_SBCategory.qryid where sbCategory = '" + listQueries + "'" +
                      "and aa_SBCategory.deleteflag is null", ClsUtility.ObjectEnum.DataRow, clsGbl.PMMSType);
          ...
          try
          {
              ...
              if (ext == ".xlsx")
              {
                ...
                if (oWs.Name.ToLower() == "lptf") oWs.Name = "2014 - 2015";  // Quick fix for exporting xlxs which needs to be imported in another system the that given sheet name : @Victor to set this dynamic
    ...
    private void getQueries ( Int32 mReport, string oParams )
    {
      ...
      theDt = (DataTable)theObject.ReturnObject ( Entity.getconnString ( clsGbl.xmlPath ), ClsUtility.theParams, "SELECT DISTINCT QryID, sbCatID, sbCategory " +
          "FROM aa_SBCategory INNER JOIN  aa_Category ON aa_SBCategory.CatID = aa_Category.CatID " +
          "WHERE (aa_SBCategory.DeleteFlag Is Null Or aa_SBCategory.DeleteFlag =0) And QryID Is Not Null And aa_Category.CatID=" + mReport, ClsUtility.ObjectEnum.DataTable, clsGbl.PMMSType );
      theDr = theDt.CreateDataReader ( );
    ...
    public XmlDocument createDHIS2XMLFileADX() 
    {
        ...
        XmlTextWriter xtw = new XmlTextWriter("C:\\Cohort\\DHIS2XMLADX.xml", System.Text.Encoding.UTF8);
        ...
        xtw.WriteAttributeString("orgUnit", DHISCode);//Facility ID
       ...
       cmd.Parameters.Add("@fromDate",SqlDbType.DateTime).Value=fromDateMOH;
       cmd.Parameters.Add("@todate", SqlDbType.DateTime).Value = ToDateMOH;
       //var stg = new StringBuilder();
       
       SqlDataAdapter da = new SqlDataAdapter(cmd);
       
       da.Fill(DHISADX);
       //DataTableReader dtr = new DataTableReader(DHISADX);
      

      // DataTableReader dtr = DHISADX.CreateDataReader();
        ...
        foreach (DataRow row in DHISADX.Rows)
        {
            if (DHISADX.Rows.IndexOf(row) != 0)
            {
                ...
                //             xtw.WriteAttributeString("value", dtr.GetInt64(0).ToString());
                //         xtw.WriteAttributeString("dataElement", dtr.GetString(1));

                // for (int i = 0; i==dtr.FieldCount; i++) {

                // xtw.WriteAttributeString(dtr.GetName(i).ToString(), dtr[i].ToString());
                //xtw.WriteAttributeString (dtr.GetOrdinal
                //  xtw.WriteAttributeString("dataElement", dtr["DataElement"].ToString());
        ...
        xld.Load("C:\\Cohort\\DHIS2XMLADX.xml");
        return xld;
    
    }

    public XmlDocument createDHIS2XMLFile ( )
    {
       ...
       MultiPartForm mf =new MultiPartForm();
        ...
        // <input type="File" name="f1">
         mf.AddSection(CURLformoption.CURLFORM_COPYNAME, "uploadedfile",
            CURLformoption.CURLFORM_FILE, "C:\\Cohort\\moh_731_adx.xml",
            CURLformoption.CURLFORM_CONTENTTYPE, "application/xml",
            CURLformoption.CURLFORM_END);

        Easy easy = new Easy();
        easy.SetOpt(CURLoption.CURLOPT_HTTPPOST, mf);
        easy.SetOpt(CURLoption.CURLOPT_URL, "http://test.phiresearchlab.org/dhis/dhis-web-dashboard-integration/index.action");
