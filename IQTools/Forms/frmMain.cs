...
namespace IQTools
{
    ...
    public partial class frmMain : Form
    {
        ...
        DataGridView dgvQueryAdherence = new DataGridView();
        ...
        private void cmdClinical_Click(object sender, EventArgs e)
        {
            ...
            else if (cboClinical.Text == "3: Excel Report: Duration in days before ART start")
            {
                //ExcelReports Er = new ExcelReports();
        ...
        private void analyzeAdherence()
        {
           
            try
            {
                DataTable results = (DataTable)dgvAdherence.DataSource;
                 //dgvQueryAdherence = new DataGridView();
                dgvQueryAdherence.ColumnCount = 2;
                dgvQueryAdherence.Columns[0].Name = "dgcDataElement";
                dgvQueryAdherence.Columns[1].Name = "dgcAvailability";
                ...
                for (int i = 0; i < results.Columns.Count; i++)
                {
                    ...
                    {
                        if (dgvAdherence[i, j].Value.ToString() != String.Empty)
                        { availability += 1; }
                    }
                    if (rowCount > 0)
                    {
                    ...
                    dgvQueryAdherence.Rows.Add(results.Columns[i].ColumnName, availabilityPercent);
                    availability = 0;
                    //if (dgvQueryAdherence.InvokeRequired)
                    //{
                    ...
                    //}
                    //else
                    //{ dgvQueryAdherence.Rows.Add(results.Columns[i].ColumnName); }
                }
        ...
        private void cmdART_Click(object sender, EventArgs e)
        {
            ...
            if (dgvAdherence.DataSource != null && cmdART.Text == Assets.Messages.ExportToExcel)
            {
                ...
                try
                {
                    Thread exportDataThread = new Thread(() => exportDataToExcel((DataTable)dgvQueryAdherence.DataSource));
        ...
        private void addCustomFilterGUI(string reportName)
        {
            ...
            dt = (DataTable)theObject.ReturnObject(Entity.getconnString(clsGbl.xmlPath), ClsUtility.theParams,
                   "SELECT ReportParamID, ParamName, ParamType" +
                    " FROM  aa_reportparameters JOIN aa_CustomReports On aa_CustomReports.reportid=aa_reportparameters.ReportID where ReportName = '" + reportName +
                    "' and (aa_reportparameters.Deleteflag <>1 or aa_reportparameters.deleteflag is null)",
                    ClsUtility.ObjectEnum.DataTable, serverType);
        ...
        private void lblDBVersion_Click(object sender, EventArgs e)
        {
